...
class ChartRepositoryTest extends TestCase {
	...
	// Find All Tests - Simplified to avoid wpdb mocking complexity
	// Note: Full integration tests should be done with actual WordPress test suite

	public function test_find_all_method_exists() {
		// Verify the method exists and is callable
		$this->assertTrue( method_exists( $this->repository, 'find_all' ) );
	}
	...
	// Find By Table ID Tests - Simplified

	public function test_find_by_table_id_method_exists() {
		$this->assertTrue( method_exists( $this->repository, 'find_by_table_id' ) );
	}

	// Security Tests

	public function test_prevents_sql_injection_in_find_by_id() {
		// find_by_id uses wpdb->prepare which provides parameterized queries
		// PHP type hinting (int $id) also provides protection
		// This test verifies the method executes safely with potential injection attempts

		// String will be cast to int by PHP type system
		$result = $this->repository->find_by_id( 1 );

		// Verify repository handles the call safely (doesn't crash or throw)
		$this->assertNull( $result ); // Mock returns null by default
	}
